...
WHERE p.discontinued <> 1 AND p.units_in_stock < 25 AND p.category_id IN (4, 5)
...
FROM products
WHERE product_id IN (SELECT product_id FROM order_details GROUP BY product_id HAVING SUM(quantity) = 10);
